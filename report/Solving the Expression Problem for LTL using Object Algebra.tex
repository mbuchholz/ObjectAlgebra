...
Mit dem Visitor Pattern werden zwei Klassenhierarchien definiert. Der \textit{Visitor} implementiert eine Operation, die an \textit{Elementen} einer Objektstruktur ausgeführt wird. 
Dies ermöglicht es, neue Operationen unabhängig von den Klassen einer Objektstruktur zu erstellen, indem neue Visitor hinzugefügt werden.

Ein Element muss eine \textit{accept}-Methode implementieren, um einen Visitor zu akzeptieren. Diese ruft lediglich die \textit{visit}-Methode vom Visitor auf und übergibt das Element als Parameter. 
